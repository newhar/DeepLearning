...
# *hint* dropout 확률을 위한 placeholder -> Done
...
l2_norm = tf.nn.l2_loss(W1)
l2_norm += tf.nn.l2_loss(b1)
# *hint* tf.nn.dropout으로 dropout 적용 -> Done
# *hint* tf.nn.l2_loss로 parameter들의 l2_norm 값 누적 -> Done

W2 = tf.get_variable("W2", shape=[300, 200], initializer=tf.contrib.layers.xavier_initializer())  # *hint* weight initialization을 위해 "initializer" 파라미터에 특정 초기화 기법을 입력
b2 = tf.Variable(tf.random_normal([200]))
L2 = tf.nn.relu(tf.matmul(L1, W2) + b2)
L2 = tf.nn.dropout(L2, keep_prob=keep_prob)
l2_norm += tf.nn.l2_loss(W2)
l2_norm += tf.nn.l2_loss(b2)
# *hint* tf.nn.dropout으로 dropout 적용 -> Done
# *hint* tf.nn.l2_loss로 parameter들의 l2_norm 값 누적 -> Done

W3 = tf.get_variable("32", shape=[200, 100], initializer=tf.contrib.layers.xavier_initializer())
b3 = tf.Variable(tf.random_normal([100]))
L3 = tf.nn.relu(tf.matmul(L2, W3) + b3)
L3 = tf.nn.dropout(L3, keep_prob=keep_prob)
l2_norm += tf.nn.l2_loss(W3)
l2_norm += tf.nn.l2_loss(b3)
# *hint* tf.nn.l2_loss로 parameter들의 l2_norm 값 누적 -> Done
...
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=hypothesis, labels=Y)) + 0.001 * l2_norm #hypothesis를 softmax를 통해 확률값으로 변형, Y와 비교해 cross_entropy error 계산
# *hint* cost에 weight decay 적용 -> Done
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        feed_dict = {X: batch_xs, Y: batch_ys, keep_prob: 0.7} # placeholder에 실제 data를 먹여주기 위한 dictionary
        # *hint* dropout 확률을 placeholder에 추가 -> Done
    ...
    val_accuracy, summaries = sess.run([accuracy, summary_op], feed_dict={X: mnist.validation.images, Y: mnist.validation.labels, keep_prob: 1})
    # *hint* dropout 확률을 placeholder에 추가 -> Done
