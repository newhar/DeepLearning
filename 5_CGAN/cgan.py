...
class CGAN(object):
    ...
    def __init__(self, config):
        ...
        # discriminator의 weight matrix들을 초기화, X1 : 784 + self.Y_dim
        ...
        # generator의 weight matrix들을 초기화, X2 : self.Z_dim + self.Y_dim
        ...
        self.G_sample = self.generator(self.input_z, self.input_y) # generator가 생성한 image, X3 : self.input_z
        D_real, D_logit_real = self.discriminator(self.input_x, self.input_y) # discriminator의 실제이미지에 대한 예측 , X4 : self.input_x
        D_fake, D_logit_fake = self.discriminator(self.G_sample, self.input_y) # discriminator의 가짜이미지에 대한 예측 , X5 : self.G_sample
    ...
    def generator(self, z, y): # generator에서 affine 연산 및 activation, X6 : [z,y]
    ...
    def discriminator(self, x, y): # discriminator에서 affine 연산 및 activation, X7 : [x,y]
